...
pacman::p_load(tidyverse, gifski, gganimate, gapminder, av)
```
...
- **#1:** Because more money - more inflation.
- **#2:** Let's find out.
```{r}
...
# Let's just recycle the code used before.
...
anim3 <- ggplot(gapminder, aes(gdpPercap, lifeExp, size = pop, color = continent)) +
  ...
  exit_shrink() +
  ease_aes('linear') + 
  labs(title = 'Year: {round(frame_time,0)}') + # Adding an animated title, how cool!
  ylab("Life expectancy (years)") +
  xlab("GDP per capita (constant 2011 international $)") + 
  labs(color = "Continent", size = "Population size") +
  scale_y_continuous(n.breaks = 15)
...
- **#7:**
Whilst coloring the dots in the graph by continent gives an idea of how life expectancy develops within each continent, it's still not the most optimal way to visualize that data, let's try to do it using a good old barplot.
```{r}

df_life_excp = gapminder %>%  
  group_by(continent,year) %>%
  summarize(mean(lifeExp))

colnames(df_life_excp)[3] = "mean_life_exp"

anim4 = ggplot(df_life_excp, aes(continent,mean_life_exp, fill = continent))+
  geom_col() +
  transition_time(year) +
...
anim4
```
Much better! It shows that the world is indeed becoming a better place, however, it seems that mean life expectancy plato's for Africa around early 90s. Intersting!
